...
class StereoDataSet:
    ...
    def rectify_stereo_pair(self, index):
        left_image = cv2.imread(self.left_images[index], 0)
        right_image = cv2.imread(self.right_images[index], 0)
        ...
        cv2.imwrite(self.main_data_dir+ "/cam0_rectified/"+str(self.get_timestamp(index))+".png", left_rectified)
        cv2.imwrite(self.main_data_dir+ "/cam1_rectified/"+str(self.get_timestamp(index))+".png", right_rectified)

        return StereoPair(left_image, right_image, left_rectified, right_rectified)

    def stereo_intrinsics_to_file(self):
        f = open(self.main_data_dir + "/cam_intrinsics_rectified.txt", "w");
        mat = self.rectified_camera_matrix;
        f.write("fx "+ str(mat[0,0])+ "\n");
        f.write("fy "+ str(mat[1,1])+ "\n");
        f.write("cx "+ str(mat[0,2])+ "\n");
        f.write("cy "+ str(mat[1,2])+ "\n");
        f.write("bline "+ str(self.stereo_baseline));
        f.close()

    def stereo_extrinsics_to_file(self):
        f = open(self.main_data_dir + "/cam_extrinsics.txt", "w");
        mat = self.rectified_camera_matrix;
        f.write("cam0");
        #writing rowwise
        for x in np.nditer(self.stereo_calibration.tr_base_left):
            f.write(' '+ str(x));
        f.write('\n');
        f.write("cam1");
        for x in np.nditer(self.stereo_calibration.tr_base_right):
            f.write(' '+ str(x));
        f.write('\n');
